import {HttpDelete, HttpPost, HttpPut, Route, RoutePrefix} from '../decorators/route.decorator';
...
export class LabelController {
	...
	getLabel(req: Request, res: Response, next: Next) {
		DatabaseService.db.collection(Label.name).findOne<Label>({_id: req.params.id}, (error, label) => {
			if (error) {
				res.send(500);
			} else {
				res.send(label);
